...
@sprites["bg"].bitmap = RPG::Cache.picture("homebg")
...
@sprites["gendergirl"].ox = @barwidth / 2
@sprites["genderboy"].ox = @barwidth / 2
@sprites["genderboy"].opacity = 180
@sprites["gendergirl"].opacity = 180
...
#===================================================
  ...
  @sprites["girl"].x = Graphics.width * 0.75
  ...
  @sprites["boy"].x = Graphics.width * 0.25
...
@sprites["genderboy"].z = 1
@sprites["gendergirl"].z = 1
...
if !pbConfirmMessage(_INTL("¿Deseas elegir esta apariencia?"))
22.times do
Graphics.update
@sprites["gendergirl"].x -= 16
@sprites["girl"].x -= 12
...
@sprites["msgwindow"].visible = true
  ...
  pbMessage("Introduce un identificador.")
  selectName
end
...
pbChangePlayer(2)
@sprites["msgwindow"].visible = false
pbMessage("Identidad visual registrada.")
pbMessage("Introduce un identificador.")
selectName
pbMessage("Registro finalizado.")
    end
...
if WRITETRAINERNAME
  pbHide
  pbTrainerName
  pbMessage(_INTL("Identificador: {1}", $player.name))
  if !pbConfirmMessage(_INTL("¿Confirmar?"))
    selectName
  else
    @finished = true
  end
