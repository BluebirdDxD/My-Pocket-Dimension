    ...
    2.times do |side|
      ...
      when 0
        @battle.allSameSideBattlers.each_with_index do |b, i|
          ...
          if idxSide == side && idxPoke == i
            ...
            imagePos.push([@path + "select_cursor", bgX + 60, iconY - 28, 0, 52, 166, 52])
          else
            base, shadow = BASE_DARK, SHADOW_DARK
            imagePos.push([@path + "select_cursor", bgX + 60, iconY - 28, 0, 0, 166, 52])
          end
          @sprites["info_icon#{b.index}"].x = iconX
          @sprites["info_icon#{b.index}"].y = iconY
          pbSetWithOutline("info_icon#{b.index}", [iconX + 60, iconY, 300])
          imagePos.push([@path + "info_owner", bgX + 36 + 60, iconY + 12, 0, 0, 128, 20],
                        [@path + "info_gender", bgX + 148 + 60, iconY - 34, b.gender * 22, 0, 22, 22])
          textPos.push([_INTL("{1}", b.pokemon.name), nameX + 60, iconY - 16, :center, base, shadow],
                        [@battle.pbGetOwnerFromBattlerIndex(b.index).name, nameX - 10 + 60, iconY + 14, 2, BASE_LIGHT, SHADOW_LIGHT])
        end
      ...
      when 1
        @battle.allOtherSideBattlers.reverse.each_with_index do |b, i|
          ...
          @sprites["info_icon#{b.index}"].x = iconX
          @sprites["info_icon#{b.index}"].y = iconY
          pbSetWithOutline("info_icon#{b.index}", [iconX + 60, iconY, 400])
          textPos.push([_INTL("{1}", b.displayPokemon.name), nameX + 60, iconY - 16, :center, base, shadow])
          if @battle.trainerBattle?
            imagePos.push([@path + "info_owner", bgX + 36 + 60, iconY + 12, 0, 0, 128, 20])
            textPos.push([@battle.pbGetOwnerFromBattlerIndex(b.index).name, nameX - 10 + 60, iconY + 14, :center, BASE_LIGHT, SHADOW_LIGHT])
          end
          imagePos.push([@path + "info_gender", bgX + 148 + 60, iconY - 36, b.displayPokemon.gender * 22, 0, 22, 22]) if !b.isRaidBoss?
        end
        if @battle.opponent
