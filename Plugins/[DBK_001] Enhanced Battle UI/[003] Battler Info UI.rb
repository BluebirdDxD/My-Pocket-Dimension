...
class Battle::Scene
  ...
  def pbUpdateBattlerInfo(battler, effects, idxEffect = 0)
    ...
    imagePos.push([@path + "info_gender", xpos + 168, ypos + 100, poke.gender * 22, 0, 22, 22]) if !battler.isRaidBoss?
    textPos  = [
      [_INTL("{1}", poke.name), iconX + 82, iconY - 20, :center, Color.new( 82, 73, 67), Color.new( 166, 153, 128)],
      [_INTL("{1}", level), xpos + 38, ypos + 104, :left, Color.new( 166, 153, 128), Color.new( 82, 73, 67)],
      [_INTL("Usó: {1}", movename), xpos + 349, ypos + 104, :center, Color.new( 166, 153, 128), Color.new( 82, 73, 67)],
      [_INTL("Turno {1}", @battle.turnCount + 1), Graphics.width - xpos - 28, ypos + 8, :center, Color.new( 82, 73, 67), Color.new( 166, 153, 128)]
    ]
    #---------------------------------------------------------------------------
    # Battler icon.
    ...
    if !battler.wild?
      ...
      textPos.push([@battle.pbGetOwnerFromBattlerIndex(battler.index).name, xpos + 32, ypos + 8, :center,Color.new( 82, 73, 67), Color.new( 166, 153, 128) ])
    end
    # Battler HP.
    if battler.hp > 0
    ...
    if battler.pbOwnedByPlayer?
      ...
      textPos.push(
        [_INTL("Hab."), xpos + 272, ypos + 44, :center, Color.new( 166, 153, 128) , Color.new( 82, 73, 67)],
        [_INTL("Obj."), xpos + 272, ypos + 68, :center, Color.new( 166, 153, 128) , Color.new( 82, 73, 67)],
        [_INTL("{1}", battler.abilityName), xpos + 376, ypos + 44, :center, Color.new( 82, 73, 67), Color.new( 166, 153, 128)],
        [_INTL("{1}", battler.itemName), xpos + 376, ypos + 68, :center, Color.new( 82, 73, 67), Color.new( 166, 153, 128)],
        [sprintf("%d/%d", battler.hp, battler.totalhp), iconX + 74, iconY + 12, :center, Color.new( 166, 153, 128) , Color.new( 82, 73, 67)]
      )
    end
    #---------------------------------------------------------------------------
  ...
  def pbAddStatsDisplay(xpos, ypos, battler, imagePos, textPos)
    [[:ATTACK,          _INTL("Ataque")],
     ...
     _INTL("Críticos")
      ...
      #-------------------------------------------------------------------------
      # 1. COLORES POR DEFECTO (NEUTROS)
      #-------------------------------------------------------------------------
      # Estos se usan si la naturaleza no afecta a la stat
      uso_base   = Color.new(186, 173, 148)
      uso_sombra = Color.new(102, 93, 87)
      ...
      if stat.is_a?(Array)
        if battler.pbOwnedByPlayer?
          battler.pokemon.nature_for_stats.stat_changes.each do |s|
            if stat[0] == s[0]
              ...
              if s[1] > 0 
                #---------------------------------------------------------------
                # 2. SI LA STAT ESTÁ SUBIDA (Naturaleza favorable)
                #---------------------------------------------------------------
                uso_base   = Color.new(230, 116, 116)
                uso_sombra = Color.new(228, 66, 66)   
                
              elsif s[1] < 0 
                #---------------------------------------------------------------
                # 3. SI LA STAT ESTÁ BAJADA (Naturaleza desfavorable)
                #---------------------------------------------------------------
                uso_base   = Color.new(101, 150, 250)
                uso_sombra = Color.new(60, 120, 252)  
              end
        ...
        # Dibuja el texto usando las variables que calculamos arriba
        textPos.push([stat[1], xpos + 16, ypos + 138 + (i * 24), :left, uso_base, uso_sombra])
        stage = battler.stages[stat[0]]
      else
        # Caso especial para "Críticos" (Siempre usa los neutros)
        textPos.push([stat, xpos + 16, ypos + 138 + (i * 24), :left, uso_base, uso_sombra])
