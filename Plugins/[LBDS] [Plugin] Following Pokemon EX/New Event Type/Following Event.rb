...
class Game_FollowingPkmn < Game_Follower
  ...
  def location_passable?(x, y, direction)
    ...
    #-----------------------------------------------------------------------------
  # Método para calcular la dirección hacia una coordenada (Soluciona el NoMethodError)
  #-----------------------------------------------------------------------------
  def pbGetDirectionTo(x, y)
    return 0 if @x == x && @y == y
    if (@x - x).abs > (@y - y).abs
      return (@x > x) ? 4 : 6
    else
...
# Don't interrupt movement unless leader has moved significantly
    ...
    old_terrain = $map_factory.getTerrainTag(self.map.map_id, self.x, self.y) if self.x && self.y
     # Arreglo escaleras y mapas conectados - No se como se arreglo con esto pero se arreglo xd - si quieres usar el codigo usalo 
