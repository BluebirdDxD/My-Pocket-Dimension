...
def self.should_use_swimming_sprites?
  ...
  return false if !FollowingPkmn.waterborne_follower?
  return false if !FollowingPkmn.has_swimming_sprite?

  # Si el jugador está surfeando, usar sprite nadando
  return true if $PokemonGlobal.surfing

  # Si no está surfeando, revisar si el follower está en agua
...
EventHandlers.add(:on_player_step_taken, :update_following_pkmn_surf_sprite, proc {
...
EventHandlers.add(:on_player_step_taken, :restore_follower_after_surf, proc {
  next if !FollowingPkmn.can_check?
  next if $PokemonGlobal.surfing

  event = FollowingPkmn.get_event

  if !event || event.character_name == ""
