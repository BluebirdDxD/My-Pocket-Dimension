module FollowingPkmn
  ...
  def self.toggle(forced = nil, anim = nil)
    ...
    new_active = $PokemonGlobal.follower_toggled
    FollowingPkmn.refresh(anim)
    # Mark follower as just appeared AFTER refresh (so event exists) but BEFORE move_followers
    if !anim_1 && new_active
      event = FollowingPkmn.get_event
      event&.mark_just_appeared if event&.respond_to?(:mark_just_appeared)
    end
