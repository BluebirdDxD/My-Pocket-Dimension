...
class DiegoWTsStarterSelection
  ...
  def initialize(pkmn1, pkmn2, pkmn3)
    ...
    @sprites["shadow_1"] = IconSprite.new(@center_x - @spacing - 46, @ball_y + 24, @viewport)
@sprites["shadow_2"] = IconSprite.new(@center_x - 46,            @ball_y + 24, @viewport)
@sprites["shadow_3"] = IconSprite.new(@center_x + @spacing - 46, @ball_y + 24, @viewport)
    ...
    @sprites["hlight_1"] = IconSprite.new(@center_x - @spacing - 34, @ball_y - 34, @viewport)
@sprites["hlight_2"] = IconSprite.new(@center_x - 34,            @ball_y - 34, @viewport)
@sprites["hlight_3"] = IconSprite.new(@center_x + @spacing - 34, @ball_y - 34, @viewport)
...
@sprites["ball_1"] = IconSprite.new(@center_x - @spacing, @ball_y, @viewport)
@sprites["ball_2"] = IconSprite.new(@center_x,            @ball_y, @viewport)
@sprites["ball_3"] = IconSprite.new(@center_x + @spacing, @ball_y, @viewport)
...
@sprites["select"] = IconSprite.new(@center_x, @ball_y, @viewport)
...
# HAND
@sprites["selection"] = IconSprite.new(@center_x - 28, @ball_y - 154, @viewport)
...
@sprites["selection"].visible = false
    ...
    @sprites["pkmn_1"].x = StarterSelSettings::STARTER1X + @center_x
    @sprites["pkmn_1"].y = StarterSelSettings::STARTER1Y + (@center_y - 40)
    @sprites["pkmn_2"].x = StarterSelSettings::STARTER2X + @center_x
    @sprites["pkmn_2"].y = StarterSelSettings::STARTER2Y + (@center_y - 40)
    @sprites["pkmn_3"].x = StarterSelSettings::STARTER3X + @center_x
    @sprites["pkmn_3"].y = StarterSelSettings::STARTER3Y + (@center_y - 40)
...
@sprites["textwnd"] = IconSprite.new(0, 0, @viewport)
...
@sprites["textwnd"].y = Graphics.height - @sprites["textwnd"].bitmap.height
    ...
    if StarterSelSettings::INSTYLE == 2 # Checks for interface style
      @sprites["textbox"].baseColor = Color.new(16, 24, 32)
      @sprites["textbox"].shadowColor = Color.new(168, 184, 184)
    else
      @sprites["textbox"].baseColor = Color.new(16, 24, 32)
      @sprites["textbox"].shadowColor = Color.new(168, 184, 184)
    end
  ...
  def pbStartChoosing
    # Starts the scene
@x = [nil, @center_x - @spacing, @center_x, @center_x + @spacing]
@y = [nil, @ball_y, @ball_y, @ball_y]
    while @endscene == 0
  ...
  def pbChoosingScene
    ...
    while @endscene == 0
      ...
      if Input.trigger?(Input::RIGHT) && @select < 3
        ...
        @frame = 0
        pbPlayCursorSE
@sprites["ball_1"].x = @center_x - @spacing if @oldsel == 1
@sprites["ball_2"].x = @center_x            if @oldsel == 2
@sprites["ball_3"].x = @center_x + @spacing if @oldsel == 3

@sprites["hlight_1"].x = @center_x - @spacing - 34 if @oldsel == 1
@sprites["hlight_2"].x = @center_x - 34            if @oldsel == 2
@sprites["hlight_3"].x = @center_x + @spacing - 34 if @oldsel == 3

@sprites["shadow_1"].x = @center_x - @spacing - 46 if @oldsel == 1
@sprites["shadow_2"].x = @center_x - 46            if @oldsel == 2
@sprites["shadow_3"].x = @center_x + @spacing - 46 if @oldsel == 3
